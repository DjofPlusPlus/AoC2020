...
namespace AoC6
{
	class Program
	{
		...
		static void Problem1(string[] groups)
		{
			Console.WriteLine("#1: " + groups
				.Select(g => g
					.Replace("\r\n", "")		// Remove new lines within group
					.GroupBy(c => c)			// Aggregate by char to gather duplicates
					.Count())					// Count the number of instances by char
				.Sum());
		}

		static void Problem2(string[] groups)
		{
			Console.WriteLine("#2: " + groups
				.Select(g => g
					.ToLookup(c => c))			// Aggregate by char, but keep newlines (count of newlines +1 == number of group members), and allow direct lookup
				.Select(gl => gl				// Within each group, find char/answers where number of answers == number of members (\n)
					.Where(cg => !"\r\n".Contains(cg.Key) && cg.Count() == gl['\n'].Count()+1)
					.Count())					// Count the number of selected answers
				.Sum());
		}
