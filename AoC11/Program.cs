...
namespace AoC11
{
	class Program
	{
		static void Main(string[] args)
		{
			...
			var seats = LinesTo2D(lines);

			Problem1((char[,])seats.Clone());
			Problem2((char[,])seats.Clone());
			Console.ReadLine();
		}

		static void Problem1(char[,] seats)
		{
			seats = RunGameOfSeats(seats, CountImmediateNeighbors, 4);
			Console.WriteLine("#1: " + seats.Cast<char>().Where(c => c == '#').Count());
		}
		...
		static int CountImmediateNeighbors(char[,] seats, int y, int x)
		{
			int found = 0;
			for (var j = Math.Max(0, y - 1); Math.Min(y + 1, seats.GetUpperBound(0)) >= j; ++j)
			for (var i = Math.Max(0, x - 1); Math.Min(x + 1, seats.GetUpperBound(1)) >= i; ++i)
			{
				if (y == j && x == i)
					continue;
				if (seats[j, i] == '#')
		...
		static int CountVisibleDiagonaly(char[,] seats, int y, int x)
		{
			int found = 0;
			foreach (var offsets in directionOffsets)
			{
				int j = y + offsets[0];
				int i = x + offsets[1];
				for (; 0 <= j && 0 <= i && seats.GetLength(0) > j && seats.GetLength(1) > i; j += offsets[0], i += offsets[1])
				{
					if (seats[j, i] == '#')
						found++;
					if (seats[j, i] != '.')
		...
		static char[,] LinesTo2D(string[] lines)
		{
