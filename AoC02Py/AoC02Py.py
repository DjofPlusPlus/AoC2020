...
rex = re.compile(r'(?P<lc>\d+)-(?P<hc>\d+)\ (?P<c>\w)\:\ (?P<p>\w+)')

def problem1(sinputs):
	valid = 0
	for i in sinputs:
		match = rex.match(i)
		count = match.group("p").count(match.group("c"))
		lc = int(match.group("lc"))
		hc = int(match.group("hc"))
		if lc <= count and count <= hc:
			valid += 1
	print("Problem 1: ", valid)

def problem2(sinputs):
	valid = 0
	for i in sinputs:
		match = rex.match(i)
		c = match.group("c");
		...
		lc = int(match.group("lc"))
		hc = int(match.group("hc"))
		if (p[lc - 1] == c) != (p[hc - 1] == c):
			valid += 1
	print("Problem 2: ", valid)
	
problem1(input.split("\n"))
problem2(input.split("\n"))
