...
namespace AoC7
{
	class Program
	{
		...
		{
			string[] bags = input.Split(new string[] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries);
			Problem1(bags);
			Problem2(bags);
			Console.ReadLine();
		}

		static void Problem1(string[] bags)
		{
			var rules = ParseParentsRules(bags);
			Console.WriteLine("#1: " + RecurseFindRootBags(rules, "shiny gold")
				.Count());
		}

		static void Problem2(string[] bags)
		{
			var rules = ParseChildrenRules(bags);
			Console.WriteLine("#2: " + RecurseFindAllChildrenBags(rules, Tuple.Create("shiny gold", 1), 0)
				.Select(ct => ct.Item2)
				.Sum());
		}

		static IEnumerable<string> RecurseFindRootBags(ILookup<string, Match> parentRules, string bag)
		{
			var parents = parentRules[bag]
				.Select(i => i.Groups["b"].Value)
				.Distinct();
			return parents
				.Concat(parents.SelectMany(p => RecurseFindRootBags(parentRules, p)))
				.Distinct();
		}

		static IEnumerable<Tuple<string, int>> RecurseFindAllChildrenBags(ILookup<string, Match> childrenRules, Tuple<string, int> parent, int dept)
		{
			var childrenTuples = childrenRules[parent.Item1]
				.SelectMany(p => p.Groups["c"].Captures.Zip(p.Groups["n"].Captures))						// Expand selection to children, pair children bag count and name
				.Select(ccp => Tuple.Create(ccp.First.Value, int.Parse(ccp.Second.Value) * parent.Item2));  // Multiply the number of children by the number of parents, since they compound...
			return childrenTuples
				.Concat(childrenTuples.SelectMany(ct => RecurseFindAllChildrenBags(childrenRules, ct, dept + 1)));
		}

		static ILookup<string, Match> ParseParentsRules(string[] bags)
		{
			return bags
				.Select(b => bagRex.Match(b))																// Parse row
				.SelectMany(m => m.Groups["c"].Captures.Select(c => new { Capture = c, Match = m }))		// Expand selection from rows to multi-children
				.ToLookup(v => v.Capture.Value, v => v.Match);												// Lookup based on children name, regex match as value
		}

		static ILookup<string, Match> ParseChildrenRules(string[] bags)
		{
			return bags
				.Select(b => bagRex.Match(b))																// Parse row
				.ToLookup(m => m.Groups["b"].Value, m => m);												// Lookup based on parent name, regex match as value
		}

		static Regex bagRex = new Regex(@"((?<b>[\w\ ]+)\ bags)( contain)((?<nc>\ (?<n>\d+)\ (?<c>[\w\ ]+)\ bag(s)?[\,\.])+|\ no other bags.)", RegexOptions.Compiled | RegexOptions.CultureInvariant | RegexOptions.ExplicitCapture);
