...
namespace AoC18
{
	class Program
	{
		...
		{
			var lines = input.Replace(" ", "").Split(new char[] { '\n', '\r' }, StringSplitOptions.RemoveEmptyEntries);
			Problem1(lines);
			Problem2(lines);
			Console.ReadLine();
		}
		...
		{
			long sum = lines
				.Select(l => Regex.Split(l, @"([\+\-\*\/\(\)])").Where(s => s != "").ToArray())
				.Select(eq => SolveLeft(eq, 0, eq.Length))
				.Sum();
		...
		static void Problem2(string[] lines)
		{
			long sum = lines
				.Select(l => Regex.Split(l, @"([\+\-\*\/\(\)])").Where(s => s != "").ToArray())
				.Select(eq => SolvePrecedence(eq, 0, eq.Length, out int unused))
				.Sum();
			Console.WriteLine("#2: " + sum);
		}

		static long SolveLeft(string[] eq, int index, int end)
		{
			long result = "(" == eq[index] ?
					SolveLeft(eq, index + 1, (index = FindClosingBracket(eq, index + 1, end))) :
					int.Parse(eq[index]);
			while (end > ++index)
			{
				char op = eq[index++][0];
				long rightVal = "(" == eq[index] ?
					SolveLeft(eq, index + 1, (index = FindClosingBracket(eq, index + 1, end))) :
					int.Parse(eq[index]);
				switch (op)
				{
		...
		static long SolvePrecedence(string[] eq, int index, int end, out int subEnd)
		{
			long result = "(" == eq[index] ?
					SolvePrecedence(eq, index + 1, FindClosingBracket(eq, index + 1, end), out index) :
					int.Parse(eq[index++]);
			while (end > index)
			{
				char op = eq[index++][0];
				switch (op)
				{
					case '+':
						...
						break;
					case '*':
						result *= SolvePrecedence(eq, index, end, out index);
						break;
				}
			}
		...
		static int FindClosingBracket(string[] eq, int index, int end)
		{
			int nest = 0;
			for (; Math.Min(eq.Length, end) > index; ++index)
			{
				switch (eq[index][0])
				{
					case '(': ++nest; break;
