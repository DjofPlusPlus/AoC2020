...
namespace AoC4
{
	class Program
	{
		...
		{
			Console.WriteLine("#1 Valid Count: " + entries.Where(IsPassportEntryFieldsValid).Count());
		}

		static void Problem2(string[] entries)
		{
			Console.WriteLine("#1 Valid Count: " + entries.Where(IsPassportEntryFieldsDataValid).Count());
		}

		static bool IsPassportEntryFieldsValid(string entry)
		{
			string[] fields = entry.Split(new char[] { '\r', '\n', ' ' }, StringSplitOptions.RemoveEmptyEntries);
			if (fields.Length < reqFields.Length)
				return false;

			int foundFlags = 0;
			foreach(string f in fields)
			{
				int fieldIndex = Array.IndexOf(reqFields, f[..3]);
				if (0 > fieldIndex)
					continue;
				foundFlags |= (1 << fieldIndex);
			}

			return 0x7f == foundFlags;
		}

		static bool IsPassportEntryFieldsDataValid(string entry)
		{
			string[] fields = entry.Split(new char[] { '\r', '\n', ' ' }, StringSplitOptions.RemoveEmptyEntries);
			if (fields.Length < reqFields.Length)
				return false;
			...
			foreach (string f in fields)
			{
				...
				if (':' != f[3])
					return false;
				if (!reqFieldsDataValidators[fieldIndex](f[4..]))
					return false;
				foundFlags |= (1 << fieldIndex);
			}
		...
		static Func<string, bool>[] reqFieldsDataValidators =
		{
			...
			f =>
			{
				...
				if (f.Length != 4)
					return false;
				if (!int.TryParse(f, NumberStyles.Integer, null, out value))
					return false;
			...
			f =>
			{
				...
				if (f.Length <= 2)
					return false;
				if (!int.TryParse(f[..^2], NumberStyles.Integer, null, out value))
					return false;
				switch (f[^2..])
				{
			...
			f =>
			{
				...
				if ('#' != f[0])
					return false;
				return int.TryParse(f[1..], NumberStyles.HexNumber, null, out value);
			},
			...
			f =>
			{
				if (f.Length != 3)
					return false;
				return new string[] { "amb", "blu", "brn", "gry", "grn", "hzl", "oth" }.Contains(f);
			},
			...
			f =>
			{
				...
				if (f.Length != 9)
					return false;
				return int.TryParse(f[1..], NumberStyles.Integer, null, out value);
			},
		};
